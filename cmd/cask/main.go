...
func init() {
	commands = []commandSpec{
		...
		{
			name:     "list",
			operands: "[-limit <n>] [-offset <n>] [-type <type[@major]>] [-codec <tag>] [-json]",
			summary:  "list objects ({total, objects}); filters on the envelope header; skips a stray non-object file with a warning",
		...
		},
		{
			name:     "meta",
			operands: "<hash> [-json]",
			summary:  "metadata of one object (size, type, frame version, codec, algorithm)",
		...
		},
		{
			name:     "stats",
			operands: "[-json]",
			summary:  "storage statistics (N objects, M bytes) plus a header census (per type, per frame version, per codec)",
			flags:    func() *flag.FlagSet { return statsFlags(new(statsArgs)) },
			op:       opStats,
		},
		{
			name:     "verify",
			operands: "<hash>|--all [-hash-algo <name>] [-checksums [-checksum <algo>]]",
			summary:  "integrity check (single object or full scan; --hash-algo selects the address algorithm; --checksums checks the recorded per-object checksum)",
		...
		{
			name:     "web",
			operands: "[-store <dir>] [-backend <name>] [-bind <addr>] [-hash-algo <name>] [-tokens r=t,...] [-token-file <path>] [-trusted-proxy <ip|cidr,...>] [-allow-insecure-bind] [-show-token] [-no-open]",
			summary:  "start the embedded viewer; the startup token is never logged (shown once on stdout — an interactive terminal, or -show-token — or supplied with -token-file/" + viewerTokenEnv + ")",
