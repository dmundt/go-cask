...
- **Dedup at the byte layer** — `Exists` before `Put`: identical bytes ⇒ identical hash ⇒ stored once, reported as `deduplicated`.
...
The demo prints `stored <hash> deduplicated=…`, `fetched N bytes`, and meta/stats lines.
