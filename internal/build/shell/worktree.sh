#!/usr/bin/env bash
# ARCHIVED — superseded by `go run ./cmd/buildtool worktree`.
#
# The relative `.git` link, the admin directory it must resolve to, the lock and the
# comparison that refuses a link into another tree are internal/build/core/worktree's; the
# command performs them and carries the same statuses (1 already exists, 2 a link that does
# not resolve and the prune refusal, 3 uncommitted work). Two things moved on purpose: the
# gate's own lock step now calls this command's quiet form instead of writing the lock in
# shell, so one rule owns the lock's text, and `prune` prints the refusal itself rather than
# through a wrapper. Nothing runs this copy; see ../shell/README.md.
