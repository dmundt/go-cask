...
#
# Every check below (baseline ownership, the archive-before-refresh rule, the
# same-file refusal, the exit statuses) is `cmd/buildtool/bench_test.go`, which drives
# the commands through injected collaborators instead of stubbing `go` and `benchstat`
# in throwaway repositories — the same behaviour, without a shell. This copy is the
# record of the cases the port had to keep. Nothing runs it; see ../shell/README.md.
set -euo pipefail
