#!/usr/bin/env bash
# ARCHIVED — superseded by `go run ./cmd/buildtool land-lane`.
#
# The records, the identity, the idle-time staleness rule and what an acquisition may do
# with a slot it did not write are internal/build/core/lane's; the exclusive claim, the
# rename-aside eviction and the messages are the command's. The port also fixed a race
# this script shared with it: the slot is created before the record is written into it, so
# a contender that read the slot in between took a half-written slot over — the engine now
# reports that as unreadable and waits for the record. Nothing runs this copy; see
# ../shell/README.md.
