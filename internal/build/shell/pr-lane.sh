#!/usr/bin/env bash
# ARCHIVED — superseded by `go run ./cmd/buildtool pr-lane`.
#
# The protocol is unchanged: the same compare-and-swap on `refs/lane/<NNN>`, the same
# annotated tag record naming who claimed the lane, the same 90-minute window for a claim
# with no pull request yet, and the same `release` as the deliberate exit. What the ref
# means and what a claim may do with it are internal/build/core/claim's; the `gh`, git and
# ref calls and the messages are cmd/buildtool pr-lane's; the layout it reads — the ref
# namespace, the window, the override variables, the local record file — is
# internal/build/policy's PRLane table.
#
# One thing moved on purpose: `status --json` reports `"state":"unreadable"` for a ref whose
# claim record cannot be read, where this script left the state at its `"stale"` default.
# The readable line is unchanged ("stale: unreadable claim record"), so only the machine
# field distinguishes a record that cannot be read from a claim that has aged out — two
# different states in the verdict (free|held|claiming|stale|unreadable).
#
# Nothing runs this copy; its cases against a stub `gh` are ordinary Go tests now
# (cmd/buildtool/prlane_test.go). See ../shell/README.md.
...
#
# The local advisory slot (`go run ./cmd/buildtool land-lane`) is a different thing
# and stays a different thing: it keeps two gate runs in ONE clone from overlapping,
# so a clone does not pay twice for the same tree. It is not the lane, and
