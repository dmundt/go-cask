...
> mirror the user-facing `CHANGELOG.md` entries into the release notes, include
> the full changelog link used by prior releases (`Full Changelog:` + compare URL),
...
> `verify` gate (gofmt, module drift, import boundary checks, `gitlike` codec
> guard, race+coverage checks, fuzz smoke, doc integrity) and the
> `platform-matrix` job across Linux/Windows/macOS targets. Treat this as a
