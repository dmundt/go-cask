...
- `-algo <name>` selects the write algorithm (default `sha256`); reads accept any registered algorithm (cas-core §4.2).
...
- Flags: single-dash long names (`-store`, `-algo`, `-json`, `-o`, `-min-age`, `-dry-run`, `-limit`, `-offset`, `-bind`, `-tokens`, `-allow-insecure-bind`, `-config` (deferred)).
