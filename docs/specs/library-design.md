...
- [x] `cas/` ≤ ~1600 code lines (906 today; blank and comment-only lines do not count) and ≤ ~45 exported identifiers — 44 today, re-checked against `go doc ./cas` in go-cask#191 (§1 budget, metric stated in go-cask#271; the checklist previously said ~1500/~20, which contradicted it)
