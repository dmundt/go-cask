...
- Only additive, non-breaking changes inside the current major; breaking changes require a major version and a migration note — **except** for a change explicitly ratified while the surface is in its first release cycle and adoption is negligible: `cas.Hash` became a concrete value type — and gave up its JSON marshalling to the JSON codec's `jsoncodec.Hash` field type — in `v1.2.0` (a documented `BREAKING CHANGE` with a migration note, versioning §4) rather than waiting for a `/v2` mirror. Any further breaking change follows the ordinary rule again.
