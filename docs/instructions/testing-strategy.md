...
**Store**
- empty store: `GetRaw`/`Get` → `ErrNotFound`; `Exists` false; `Delete`
  no-op
- `Put` of empty bytes; `PutDedup` first vs repeat; `GetRaw` vs `Get`
- type mismatch path (`Get` on a store whose `T` doesn't match the
...
- [x] every requirement ID (P-01…P-05, sentinel errors, ops, defaults,
      example-API acceptance) has at least one test, named with the ID (§2)
- [x] corner & error inventory of §3 covered per component
- [x] fuzz targets present, corpora committed, CI runs them
