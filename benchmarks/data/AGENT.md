...
Folder stores canonical benchmark JSON files + schema used to validate them.

## Rules

- Treat JSON file as source of truth for benchmark results.
- Keep schema in same directory as JSON result file it validates.
- Preserve exact field names + value shapes already used by benchmark runner.
- Benchmark matrix changes: update JSON first, then schema + any README summary that depends on it.
- Keep runner metadata (`generated_at`, `runner`, `statistic`): comparisons remain machine-specific + traceable.
- Never rename benchmark file without updating schema ID + references in benchmark docs.
