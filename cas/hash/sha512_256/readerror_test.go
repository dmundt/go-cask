...
import (
	...
	sha512256hash "github.com/dmundt/go-cask/cas/hash/sha512_256"
)
...
func TestDigestReportsAReadFailure(t *testing.T) {
	...
	d, err := sha512256hash.New().Digest(&brokenReader{boom: boom})
	if !errors.Is(err, boom) {
		t.Fatalf("Digest with a failing reader = %v, want %v", err, boom)
	}
	if !strings.Contains(err.Error(), "cas/sha512_256") {
