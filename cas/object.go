...
// Object is a self-describing, reference-aware typed object: it knows its
// versioned type name and which digests it references. The generic core never
// interprets types or references — References() is the single source of truth
// for traversal, preloading, and GC reachability.
//
// Serialization is NOT an object concern: the Store[T] is configured with a
// Codec[T], and Store.Put writes the type + codec payload to the byte layer
// itself — the codec is the single serialization authority on write and read
// (cas-core §8 decision 1). Type() must return a versioned type name
// "<type>@<major>" (e.g. "commit@1") so several object-model majors can
// coexist in one store and the version travels with the bytes.
