...
Codec layer = serialization boundary between generic CAS core + application payload format. Keep core format-agnostic, codecs explicit, every wrapper composable.

## Core rules

- Keep core store format-agnostic: storage + identity unaffected by codec choice.
- Prefer explicit encode/decode functions over runtime reflection. Package's exported API must remain typed + predictable.
- Every codec implements same public contract: `Encode(T) ([]byte, error)` + `Decode([]byte) (T, error)`.
...
- Never add hidden runtime type discovery or reflection to `cas` or `cas/*` packages. Keep conversion logic explicit + type-driven.
...
- Keep README files short + package-focused.
...
File covers codec family under `cas/codec` + package-local rules for custom codec wrappers + chainable formats.
