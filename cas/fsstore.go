...
// FSOption configures an FSBackend. Functional options per
...
// FSBackend is the filesystem Backend backend: each object is one file
...
// NewFSBackend creates a filesystem backend rooted at basePath, creating
...
func NewFSBackend(basePath string, opts ...FSOption) (*FSBackend, error) {
	...
	return &FSBackend{base: basePath, fanOut: cfg.fanOut, fanLevels: cfg.fanLevels, dirSync: cfg.dirSync}, nil
}
...
func (s *FSBackend) hashPath(h Hash) string {
...
func (s *FSBackend) Put(ctx context.Context, h Hash, r io.Reader) error {
...
func (s *FSBackend) Get(ctx context.Context, h Hash) (io.ReadCloser, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (s *FSBackend) Exists(ctx context.Context, h Hash) (bool, error) {
	if err := ctx.Err(); err != nil {
		return false, err
	}
...
func (s *FSBackend) Delete(ctx context.Context, h Hash) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	s.mu.Lock()
...
func (s *FSBackend) Size(ctx context.Context, h Hash) (int64, error) {
	if err := ctx.Err(); err != nil {
		return 0, err
	}
...
func (s *FSBackend) Clean(ctx context.Context, olderThan time.Duration) (int, error) {
	if err := ctx.Err(); err != nil {
		return 0, err
	}
...
func (s *FSBackend) List(ctx context.Context, algo string) ([]Hash, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	var hashes []Hash
...
// Lock-free.
func (s *FSBackend) Stats(ctx context.Context) (*StoreStats, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (s *FSBackend) Verify(ctx context.Context, h Hash) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	rc, err := s.Get(ctx, h)
...
func (s *FSBackend) GC(ctx context.Context, reachable map[string]bool) error {
...
func (s *FSBackend) Prune(ctx context.Context, roots []Hash, minAge time.Duration, dryRun bool) ([]Hash, error) {
