...
func storeRaw(t *testing.T, backend cas.Backend, typeName, payload string) cas.Digest {
	t.Helper()
	var buf []byte
	...
	d := sha256.Of(buf)
	if err := backend.Put(context.Background(), d, strings.NewReader(string(buf))); err != nil {
		t.Fatal(err)
	}
	return d
}
...
func TestGetEnforcesValidateOnDecode(t *testing.T) {
	ctx := context.Background()
	s, backend := newStrictStore(t)

	d := storeRaw(t, backend, "strict@1", `{"name":""}`)
	_, err := s.Get(ctx, d)
	...
	if _, err := s.GetRaw(ctx, d); err != nil {
		t.Fatalf("GetRaw = %v, want the backend bytes", err)
	}
}
...
func TestGetRejectsNilDecodedObject(t *testing.T) {
	...
	d = storeRaw(t, backend, "other@1", `null`)
	if _, err := s.Get(ctx, d); !errors.Is(err, cas.ErrCorrupt) {
...
func TestNilObjectWithInterfaceType(t *testing.T) {
	...
	d := storeRaw(t, backend, "iface@1", `null`)
	if _, err := s.Get(ctx, d); !errors.Is(err, cas.ErrCorrupt) {
