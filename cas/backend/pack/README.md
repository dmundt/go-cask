# pack — optional packfile backend

Package `pack` provides an opt-in backend for large stores that need to coalesce many small objects into pack files.
...
- `pack` is an extension over the core CAS byte contract; it is not a replacement for the digest model.
...
```go
raw, err := pack.New("./store", pack.WithEnabled(), pack.WithPackMaxBytes(64<<20))
```
