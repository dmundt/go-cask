...
- `ValidateBase`, `EnsureBase` and `CleanupTemp` are the exported pre-flight for a caller that owns the base path before a backend exists: check it, create it without opening a backend, or reclaim its `*.tmp` crash leftovers. A caller that only opens a store needs none of them — `fs.New` validates, and `Backend.Clean` sweeps.
